...
class Gerador():
    ...
    def gerar_combinacao_com_pesos(self, numeros, counter, quantidade=5, peso_minimo=0.7):
        """
        Gera combinações usando uma estratégia baseada em pesos/probabilidades.
        Prioriza números de ouro mas garante diversificação.

        Args:
            numeros: Lista de números candidatos
            counter: Contador de frequências
            quantidade: Quantas combinações gerar
            peso_minimo: Peso mínimo para incluir um número (0-1)
        """
        conjuntos = []

        # Calcula pesos normalizados (frequência relativa)
        max_freq = max(counter.values()) if counter else 1
        pesos = {num: counter.get(num, 0) / max_freq for num in numeros}

        # Ordena por peso
        numeros_ordenados = sorted(numeros, key=lambda x: pesos[x], reverse=True)

        # Separa números de ouro (peso alto) dos demais
        numeros_ouro = [n for n in numeros_ordenados if pesos[n] >= peso_minimo]
        numeros_normais = [n for n in numeros_ordenados if pesos[n] < peso_minimo]

        for i in range(quantidade * 2):  # Tenta mais para garantir diversidade
            conjunto = []

            # Estratégia: 4-5 números de ouro + 1-2 números normais para diversificar
            num_ouro_count = min(5, len(numeros_ouro), 6)
            num_normal_count = 6 - num_ouro_count

            # Seleciona números de ouro variados
            for j in range(num_ouro_count):
                idx = (i * 2 + j) % len(numeros_ouro)
                if numeros_ouro[idx] not in conjunto:
                    conjunto.append(numeros_ouro[idx])

            # Seleciona números normais para diversificar
            if num_normal_count > 0 and numeros_normais:
                for j in range(num_normal_count):
                    idx = (i + j) % len(numeros_normais)
                    if numeros_normais[idx] not in conjunto:
                        conjunto.append(numeros_normais[idx])
            ...
            # Calcula score da combinação (soma das frequências)
            score = sum(counter.get(n, 0) for n in conjunto)

            # Verifica se é válido
            if len(conjunto) == 6 and self.check_repetitions(conjunto, conjuntos):
                conjuntos.append((tuple(conjunto), score))
                if len(conjuntos) >= quantidade * 2:
                    break

        # Ordena por score e retorna os melhores
        conjuntos.sort(key=lambda x: x[1], reverse=True)
        return [c[0] for c in conjuntos[:quantidade]]

    def gerar_combinacao_maxima_frequencia(self, numeros, counter, quantidade=5):
        """
        Gera combinações que maximizam a soma das frequências dos números.
        Usa uma abordagem gulosa otimizada: sempre adiciona o número disponível
        com maior frequência que ainda não está no conjunto.
        """
        if not numeros or len(numeros) < 6:
            # Se não há números suficientes, retorna vazio
            return []

        conjuntos = []
        numeros_ordenados = sorted(numeros, key=lambda x: counter[x], reverse=True)

        # Limita o número de tentativas para evitar loops longos
        max_tentativas = min(quantidade * 3, len(numeros_ordenados))
        tentativas = 0

        # Gera múltiplas combinações variando o ponto de partida
        for inicio in range(max_tentativas):
            if len(conjuntos) >= quantidade * 2:
                break
            ...
            # Começa com um número diferente para cada combinação
            primeiro_num = numeros_ordenados[inicio % len(numeros_ordenados)]
            conjunto.append(primeiro_num)
            numeros_usados.add(primeiro_num)

            # Adiciona os 5 números restantes com maior frequência disponível
            for _ in range(5):
                melhor_num = None
                melhor_freq = -1
                ...
                for num in numeros_ordenados:
                    if num not in numeros_usados:
                        ...
                        # Pequena penalidade para números muito próximos (diversificação)
                        if len(conjunto) > 0:
                            # Evita números muito próximos (opcional)
                            min_dist = min(abs(num - n) for n in conjunto)
                            if min_dist < 3:
                                freq *= 0.9  # Pequena penalidade
                ...
                if melhor_num is not None:
                    conjunto.append(melhor_num)
                    numeros_usados.add(melhor_num)
                else:
                    ...
                    for num in numeros_ordenados:
                        if num not in numeros_usados:
                            ...
                            break
                    if not encontrado:
                        # Se não encontrou nenhum, para o loop
                        break

            # Verifica se tem 6 números antes de processar
            if len(conjunto) < 6:
                continue

            conjunto = sorted(conjunto[:6])

            # Calcula score (soma das frequências)
            score = sum(counter.get(n, 0) for n in conjunto)

            # Verifica se é válido e diversificado
            if len(conjunto) == 6:
                # Tenta com diferentes níveis de restrição
                max_repetitions = 3 if len(conjuntos) < quantidade else 4
                if self.check_repetitions(conjunto, conjuntos, max_repetitions=max_repetitions):
                    conjuntos.append((tuple(conjunto), score))
                    tentativas = 0  # Reset contador se encontrou uma combinação válida
                else:
                    tentativas += 1
            else:
                tentativas += 1

            # Se não encontrou combinação válida após muitas tentativas, para
        ...
        if conjuntos:
            ...
            # Fallback: retorna combinações simples se não conseguiu gerar
            return self.gerar_combinacao_otimizada(numeros, counter, quantidade)
    ...
    def calcular_estatisticas_distribuicao(self, counter, total_sorteios):
        """
        Calcula estatísticas descritivas da distribuição de frequências.
        """
        frequencias = [counter.get(num, 0) for num in range(1, 61)]

        media = np.mean(frequencias)
        mediana = np.median(frequencias)
        desvio_padrao = np.std(frequencias)
        variancia = np.var(frequencias)
        coef_variacao = (desvio_padrao / media * 100) if media > 0 else 0

        # Frequência esperada
        freq_esperada = (6 * total_sorteios) / 60

        # Desvio da média esperada
        desvio_medio = np.mean([abs(f - freq_esperada) for f in frequencias])

        # Números mais desviados
        desvios = [(num, abs(counter.get(num, 0) - freq_esperada))
